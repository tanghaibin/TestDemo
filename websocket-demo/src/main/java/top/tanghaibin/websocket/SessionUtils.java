package top.tanghaibin.websocket;
...
public class SessionUtils {
     ...
     * @param message
     */
    public static  void broadcast(String relationId, int userCode, String message) {
        if(clients.containsKey(getKey(relationId,userCode))) {
            Session session = get(relationId, userCode);
            if(session.isOpen()) {
                session.getAsyncRemote().sendText(message);
            } else {
                throw new NullPointerException(getKey(relationId, userCode) + "Connection does not exist");
            }
        }
    }
}